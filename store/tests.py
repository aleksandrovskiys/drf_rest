...
class ProductCreateTestCase(APITestCase):
    def test_create_product(self):
        ...
        product_attrs = {"name": "New Product", "description": "Awesome product", "price": "123.45"}
        ...
        self.assertSequenceEqual(response.data["price"], product_attrs["price"])


class ProductDestroyTestCase(APITestCase):
    def test_delete_product(self):
        initiali_product_count = Product.objects.count()

        product_id = Product.objects.all().first().id
        self.client.delete(f"/api/v1/products_generic/{product_id}/")

        self.assertEqual(Product.objects.count(), initiali_product_count - 1)
...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        response = self.client.patch(
            f"/api/v1/products_generic/{product.id}/",
            {
                "name": "New Product",
